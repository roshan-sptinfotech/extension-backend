...
async function loadPage()
{
    ...
    const response = await fetch(`/email-views/${emailId}?page=${page}`, {
        method: "POST",
    ...
    if(!response.ok)
    {
        console.log("Could not fetch email view data");
        return;
    }
    ...
    const statusContent = json.status === 0 ? `<i class="fa-solid fa-check ui-icon"></i> Not opened yet` : 
        `<i class="fa-solid fa-check-double ui-icon blue-icon"></i> opened ${json.status} time${json.status >= 2 ? "s": ""}`;
    ...
    <div class="email-item">
        <table>
            ...
            <tr>
                <th>Status</th> <td>${statusContent}</td>
            </tr>
    ...
    loadEmailViews(json.emailViews);
    createPaginationContent(json.status, 8, page);
    addShowMoreRecipientsOption();
}

function getViewTime(viewTime)
{
    const currentDate = new Date();
    const difference = (currentDate.getTime() - viewTime.getTime()) / 1000;

    //less than 1 minute
    if(difference <= 59)
        return `${Math.floor(difference)} second${difference >= 2? "s" : ""} ago`;
    ...
    //less than 28 days
    else if(difference <= 2419200 - 1)
    {
        const date = new Date(viewTime).toLocaleDateString() + ", " + convertTimeTo12Hour(new Date(viewTime));
        const days = Math.floor(difference / 60 / 60 / 24);
        return `${days} day${days >= 2 ? "s" : ""} ago  (${date})`;
    }
...
function loadEmailViews(emailViews)
{
    //This is an <ol> element and we must not remove its first child element
    const emailItems = document.querySelector(".email-items");

    for(let emailView of emailViews)
    {
        const li = document.createElement("li");
        const viewTime = getViewTime(new Date(emailView.viewTime));
        ...
        li.innerHTML = `
            <div class="email-item">
                <table>
                    ...
                    <tr>
                        <th>View Time</th> <td>${viewTime}</td>
                    </tr>
                </table>
            </div>
        `;
...
function createPaginationContent(totalViews, pageSize, currentPage)
{
    ...
    if(startingPage !== 1)
    {
        ...
        anchor.href = `/email-detail.html?emailId=${emailIdQuery}&page=${startingPage - 1}`;
    ...
    for(let i=startingPage; i<=endingPage; i++)
    {
        ...
        anchor.href = `/email-detail.html?emailId=${emailIdQuery}&page=${i}`;
    ...
    if(endingPage !== lastPageNumber)
    {
        ...
        const anchor = document.createElement("a");
        anchor.href = `/email-detail.html?emailId=${emailIdQuery}&page=${endingPage + 1}`;
        paginationDiv.appendChild(anchor);
