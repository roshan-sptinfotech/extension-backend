...
{
    const response = await fetch(`/user/emails?page=${page}`, {
        method: "POST",
...
function createPaginationContent(totalEmails, pageSize, currentPage)
{
    ...
    //Then we can draw the previous segments pagination link also
    if(startingPage !== 1)
    {
        const anchor = document.createElement("a");
        anchor.href = `/user.html?page=${startingPage - 1}&activeNav=1`;
    ...
    {
        const anchor = document.createElement("a");
        anchor.href = `/user.html?page=${i}&activeNav=1`;
        anchor.target = "_parent";
        paginationDiv.appendChild(anchor);
    ...
    //We can draw the next segment's pagination link as well
    if(endingPage !== lastPageNumber)
    {
        const anchor = document.createElement("a");
        anchor.href = `/user.html?page=${endingPage + 1}&activeNav=1`;
        anchor.target = "_parent";
        paginationDiv.appendChild(anchor);
...
async function loadPage()
{
    ...
    for(let i=0; i<response.length; i++)
    {
        ...
        const statusContent = emailData.status === 0 ? `<i class="fa-solid fa-check ui-icon"></i> Not opened yet` : 
        `<i class="fa-solid fa-check-double ui-icon blue-icon"></i> opened ${emailData.status} time${emailData.status >= 2 ? "s": ""}`;


        let li = document.createElement("li");
        li.innerHTML = `
            <a href="/email-detail.html?emailId=${emailData._id}" class="email-item-link">
    ...
    if(response.length === 0)
    {
        ...
        div.textContent = "No Tracked Mails Available";
